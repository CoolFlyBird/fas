...
class ReportBalanceService
{
     ...
     */
    public function __construct(BalanceModel $balanceModel, CurrentPeriodModel $periodModel)
    {
    ...
    {
        $this->calculateBalance($this->periodModel->getCurrentYear(), $this->periodModel->getCurrentPeriod());
    }
    ...
    {
        $s = $this->periodModel->getCurrentYear();
        $s = (int)$s + 1;
        $this->calculateBalance($s, '00');
    }
    ...
    private function getEndBalanceArray($year, $period)
    {
        ...
        $bs5 = $this->calculateArray($year, $period, []);
        //预付款项 = 预付账款(借)（1123）+ 应付账款(借)（2202）- 坏账准备_预付账款坏账准备（123102）
        $bs6 = $this->calculateArray($year, $period, []);
        //应收利息
        ...
        //固定资产
        $bs18 = $this->calculateArray($year, $period, [Subject::固定资产, -Subject::累计折旧, -Subject::固定资产减值准备]);
        //在建工程
        ...
        $bs35 = $this->calculateArray($year, $period, []);
        //预收款项 = 应收账款(贷)（1122）+ 预收账款(贷)（2203）
        $bs36 = $this->calculateArray($year, $period, []);
        //应付职工薪酬
    ...
    {
        //period 00表示年初
        $value = BalanceModel::where(['year' => $year, 'period' => '00', 'id' => $id])->value("endValue");
        if (!$value) {
            $value = 0;
        }
        return $value;
